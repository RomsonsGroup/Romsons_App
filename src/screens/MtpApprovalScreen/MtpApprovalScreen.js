...
const MtpApprovalScreen = () => {
    ...
    const MtpPendingList = async (teamEmpId, month = selectedMonth, year = selectedYear) => {
        ...
        try {
            ...
            const response = await fetch(
                `http://localhost:8091/MtpPendingList?empidd=${empid[0].emp_id}&month=${month + 1}&year=${year}`
            );
    ...
    const handleApprove = async () => {
        ...
        try {
            const response = await fetch("http://localhost:8091/MtpApprovedIdBy", {
                method: "POST",
    ...
    const MtpApprovalList = async () => {
        ...
        fetch(`http://localhost:8091/MtpApprovedList?empidd=${empid[0].emp_id}&month=${selectedMonth + 1}&year=${selectedYear}`, requestOptions)
            .then((response) => response.json())
            .then((result) => {
    ...
    const handleRejected = async () => {
        ...
        try {
            const response = await fetch("http://localhost:8091/MtpRejectedIdBy", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                    ids: selectedIds,
    ...
    const MtpRejectedlList = async () => {
        ...
        fetch(`http://localhost:8091/MtpRejectedList?empidd=${empid[0].emp_id}&month=${selectedMonth + 1}&year=${selectedYear}`, requestOptions)
            .then((response) => response.json())
            .then((result) => {
                console.log(result, 'approved list');
