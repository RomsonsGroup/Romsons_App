...
const AttendancePunchInOut = () => {
        ...
        const requestOptions = {
          ...
          fetch("https://crm.romsons.com:8080/AppVersionCheck", requestOptions)
            ...
            .then((result) => {
                ...
                if (!data.error) {
                    if (data.data[0].version_ios === "7.0.3") {
                        setModalVisible(true);
                    } else {
                        alert(`Please update your app to the latest version ${data.data[0].version_ios }`);
                    }
                }
            })
    ...
    const handleOut = async () => {
        ...
        if (hour > 19 || (hour === 19 && minute > 30)) {
            alert("You are not allowed to punch out after 7:30 PM");
            return;
        }
